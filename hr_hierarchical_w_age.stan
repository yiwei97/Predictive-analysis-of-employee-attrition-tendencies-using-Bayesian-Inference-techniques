data {
  // Define variables in data
   ...
   int<lower=0> p;
   //Mapping function - mapping observation to age group
   int<lower=1,upper=K> AgeGroup[N];
   // Variables
...
transformed parameters  {
   ...
   real mu[N];
   for (i in 1:N) {
     mu[i] = alpha[AgeGroup[i]] + beta[AgeGroup[i],1]*x[i,1] + beta[AgeGroup[i],2]*x[i,2] 
     + beta[AgeGroup[i],3]*x[i,3] + beta[AgeGroup[i],4]*x[i,4]; 
   }
}
model {
   //Prior
